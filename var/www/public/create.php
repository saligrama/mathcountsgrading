...
*/
	...
	$participants = $stparticipants = $participants_row = $regulars_row = $alternates_row = [];

	$compinfo = dbQuery_new($conn, "SELECT * FROM competition WHERE CID IN (SELECT CID FROM temp_comp)");
	if(empty($compinfo))
		$compinfo = 0;
	else
	{
		...
		$participants = dbQuery_new($conn, "SELECT * FROM competition_participants WHERE CID=:cid", ["cid" => $compinfo["CID"]]);
		$stparticipants = dbQuery_new($conn, "SELECT * FROM student_participants WHERE CID=:cid", ["cid" => $compinfo["CID"]]);
